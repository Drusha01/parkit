...
class Scanner extends Controller
{
    ...
    public function scan(Request $request ){
        ...
        $hash = null;
        if ($segments[0] === 'spaces' && $segments[1] === 'qr') {
            $type = $segments[0];
            $hash = $segments[2];
        }
        ...
        $vehicle = DB::table("users as u")
            ->select(
                ...
                'v.date_created' ,
                'v.date_updated' ,
                'l.is_approved as license_is_approved',
            )
            ->leftjoin('vehicles_v2 as v','v.id','u.default_vehicle_id')
            ->leftjoin('vehicle_types as vt','vt.id','v.vehicle_type_id')
            ->leftjoin('status as st','st.id','v.status_id')
            ->leftjoin('licenses_v2 as l','l.user_id','u.id')
            ->where('u.id','=',$data['user_id'])
            ->where('st.name','=', 'Active')
            ->first();
        if($vehicle->license_is_approved == null){
            return response()->json(['type'=>'warning','message' => 'Please create license '] , 200);
        }
        if($vehicle->license_is_approved == 0){
            return response()->json(['type'=>'warning','message' => 'License is not yet approved '] , 200);
        }
        ...
        if($vehicle && $space){
            ...
            if($rent){
                if($rent->previous_data && $rent->time_end == null){
                    ...
                    return response()->json(['type'=>'success','message' => 'Thank you, come again. Rent amount: '.($update['amount'] != 0 ? 'P'.$update['amount'] :'Paid via wallet')] , 200);
                }else{
                    $result = null;
                    ...
                    if($result){
                        return response()->json(['type'=>'success','message' => 'Success, Welcome back to parkIt']);
                    }
                }
                
                if($rent->previous_data ){
                    return response()->json(['type'=>'success','message' => 'Successfully time-out, please try again time-in later']);
                }else{
                    return response()->json(['type'=>'success','message' => 'Successfully time-in, please try again time-out later']);
                }
            }else{
                $result = DB::table('rents')
                ->insert($insert);
                ...
                return response()->json(['type'=>'success','message' => 'Success, Welcome to parkIt']);
            }
        } 
        return response()->json(['type'=>'warning','message' => 'Error.'] , 422);
    }
